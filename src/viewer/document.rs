...
use crate::render::{RenderedDoc, build_buffer, render_markdown};
...
pub(crate) fn apply_initial_theme(ui: &Ui, state: &Rc<RefCell<AppState>>) {
    ...
    apply_css(&ui.css_provider, state.zoom_level);
}
...
pub(crate) fn reload_document(ui: &Ui, state: &Rc<RefCell<AppState>>) {
    let path = state.borrow().file_path.clone();

    let (rendered, message, clear_search) = match fs::read_to_string(&path) {
        Ok(markdown) => (render_markdown(&markdown), Some("Reloaded"), false),
        Err(error) => (
            RenderedDoc {
    ...
    render_current_view(ui, &mut state, message, false);
    ui.window
...
pub(crate) fn render_current_view(
    ...
    let buffer = build_buffer(&rendered);
